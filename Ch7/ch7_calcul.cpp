...
double primary() {
	...
	case '(':
	{
		double d = expression();
		t = ts.get();
		if (t.kind != ')')error("Error : if primary starts with ( it should ends with )");
		return d;
	}
	case '8': {
		int x = t.value;
	...
	default:
		error("primary : [nums or \'(nums)\']가 나와야함");
	}
}
double term() {
	double left = primary();
	...
	while (true) {
		...
		case '/':
		{
			...
			if (d == 0)error("Error : divided by Zero");
			left /= d;
			t = ts.get();
			break;
		}
...
try {
	...
	while (cin) {
		cout << "> ";
		Token t = ts.get();
		while (t.kind == ';')t = ts.get();//; 중복된거 제거
		if (t.kind == 'q') {
			keep_window_open();
			return 0;
		}
		ts.putback(t);//;나 q가 아니라면 버퍼에 다시 넣기
		cout << "= " << expression() << '\n';	
	}
	keep_window_open("~0");
}
